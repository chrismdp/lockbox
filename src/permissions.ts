 ...
 */
function matchSpecificity(entries: string[], tool: string, arg: string): number {
  let best = -1;
...
export function checkPermissions(settingsPath?: string): string[] {
  ...
  if (promptAutoAllowed) {
    warnings.push(
      'CRITICAL: lockbox-prompt is auto-allowed — delegate actions execute without user review. Remove any Bash allow entry that covers lockbox-prompt. The approval gate only works when lockbox-prompt is in permissions.ask, not allow',
    );
  }

  if (promptDenied) {
    warnings.push(
      'Bash(*lockbox-prompt*) in deny — delegate approval prompt is blocked. Move to permissions.ask',
    );
  } else if (!promptInAsk && !promptAutoAllowed) {
    warnings.push(
      'Bash(*lockbox-prompt*) not in permissions.ask — delegate actions won\'t prompt for user review. Add to permissions.ask',
    );
  }
  ...
  if (delegateAllowed && !delegateDenied && !promptInAsk) {
    warnings.push(
      "Task(lockbox:delegate) in allow has no effect on approval — Task ignores ask rules (Permission Required: No). The prompt approval gate is the actual control point",
    );
  }
